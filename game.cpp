...
void Game::initiateRoomSequence() {
    ...
    if (room->row == 0 && room->col == 0 && room->enemies.empty()) {
        std::cout << "Congratulations, you have reached the exit and escaped the dungeon! Thanks for playing!\n";
        isGameOver = true;
        return;
    }
...
void Game::engageInCombat() {
    ...
    while (true) {
        ...
        std::cout << "The enemy " << enemy->getName() << "strikes you, and you take " << enemy->getDamage() << " damage. You now have " << player->getHealth() << " health.\n";
